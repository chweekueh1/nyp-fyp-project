...
def extract_with_tesseract(file_path: str) -> Optional[str]:
    ...
    try:
        ...
        image_formats = {
            ...
            ".gif",
            ".webp",
            ".svg",  # Treat SVG as image for OCR
        }
...
def extract_pdf_content(file_path: str) -> Optional[str]:
    """Extracts content from PDF files using Alpine-friendly tools."""
    # Try poppler-utils (pdftotext) first
    try:
        result = subprocess.run(
            ["pdftotext", file_path, "-"], capture_output=True, text=True, timeout=30
        )
        if result.returncode == 0 and result.stdout.strip():
            logger.info(
                f"Successfully extracted PDF content using pdftotext from {file_path}"
            )
            return result.stdout
        else:
            logger.warning(f"pdftotext failed for {file_path}: {result.stderr}")
    except Exception as e:
    ...
    try:
        ...
        if text_content.strip():
            logger.info(
                f"Successfully extracted PDF content using pypdf from {file_path}"
            )
            return text_content
        else:
            logger.warning(
                f"No text content extracted from PDF using pypdf: {file_path}"
            )
    except Exception as e:
