...
async def detectFileType_async(file_path):
    ...
    if guessed_file_type:
        ...
        if mime_type:
            file_extension = await asyncio.to_thread(
    ...
    if file_extension == ".zip":
        try:
            with await asyncio.to_thread(zipfile.ZipFile, file_path, "r") as z:
                ...
                if any(
                    f.startswith("ppt/") or f.startswith("ppt/_rels/")
                    for f in zip_contents
                ):
                    file_extension = ".pptx"
                elif any(
                    f.startswith("word/") or f.startswith("word/_rels/")
                    for f in zip_contents
                ):
                    file_extension = ".docx"
                elif any(
                    f.startswith("xl/") or f.startswith("xl/_rels/")
                    for f in zip_contents
                ):
                    file_extension = ".xlsx"
...
async def handle_uploaded_file(ui_state: dict) -> dict:
    ...
    if not username:
        return {
            "history": history,
            "response": "[Error] Username required for file upload",
            "debug": "No username.",
        }
    if not file_obj:
        return {
    ...
    try:
        ...
        if not llm_funcs or not llm_funcs["is_llm_ready"]():
            logging.error(
                "LLM/DB not ready in handle_uploaded_file. Backend may not be fully initialized."
            )
            return {
                "history": history,
                "response": "[Error] AI assistant is not fully initialized. Please try again later.",
        ...
        if not data_funcs or "dataProcessing" not in data_funcs:
            ...
            return {
                "history": history,
                "response": "[Error] File processing not available.",
                "debug": "Data processing not ready.",
            }
        ...
        if chat_id:
            history = history + [
                [f"[File: {getattr(file_obj, 'name', '[unknown]')}]", response]
            ]
        response_dict = {
            "history": history,
            "response": response,
            "debug": "File uploaded and processed.",
        }
...
async def do_login(username_or_email: str, password: str) -> Dict[str, str]:
    ...
    try:
        ...
        if not await check_rate_limit(username_or_email, "auth"):
            ...
            return {
                "code": "429",
                "message": f"Rate limit exceeded. Please wait {rate_limit_info['time_window']} seconds.",
            }
        ...
        if username_or_email in users:
            ...
            for username, data in users.items():
                if data.get("email", "").lower() == username_or_email_lower:
                    ...
                    user_data = data
                    print(
                        f"Found user by email: {username_or_email} -> username: {actual_username}"
                    )
                    break
        ...
        if not actual_username or not user_data:
            ...
            print(
                f"Available emails: {[data.get('email', 'N/A') for data in users.values()]}"
            )
    ...
    try:
        ...
        if meaningful_words:
            ...
            return (
                fallback.capitalize()
                if fallback
                else f"Chat {datetime.now().strftime('%H-%M')}"
            )
...
def rename_chat(old_chat_id: str, new_chat_name: str, username: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Renamed chat '{old_chat_id}' to '{new_chat_id}' for user {username}"
        )
