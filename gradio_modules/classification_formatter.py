...
def format_classification_response(classification: Dict[str, Any], 
                                 extraction_info: Dict[str, Any],
                                 file_info: Dict[str, str]) -> Dict[str, str]:
    """
...
def format_file_information(file_info: Dict[str, str], 
                          extraction_info: Dict[str, Any]) -> str:
    """Format file information with extraction details."""
    ...
    if extraction_info:
        ...
        if content:
            ...
            lines.append(f"📊 **Content:** {char_count:,} characters, {word_count:,} words")
...
def format_reasoning(reasoning: str, confidence: Optional[float] = None) -> str:
    ...
    if confidence is not None:
        ...
        lines.append(f"{conf_emoji} **Confidence:** {conf_desc} ({confidence_pct:.1f}%)")
        lines.append("")
...
def format_classification_summary(security_class: str, sensitivity: str,
                                confidence: Optional[float] = None,
                                extraction_info: Optional[Dict[str, Any]] = None) -> str:
...
def get_handling_recommendations(security_class: str, sensitivity: str) -> List[str]:
    ...
    elif 'PUBLIC' in security_upper or ('OFFICIAL' in security_upper and 'OPEN' in security_upper):
        recommendations.extend([
