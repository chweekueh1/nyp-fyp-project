...
def debug_chatbot_ui():
    ...
    try:
        ...
        print(f"✅ load_all_chats returned: {type(chats).__name__} with {len(chats)} chats")
        ...
        if hasattr(app, 'blocks'):
            ...
            for block in app.blocks.values():
                if hasattr(block, 'label'):
                    if 'chat' in str(block.label).lower() or 'message' in str(block.label).lower():
                        chatbot_components.append(f"{type(block).__name__}: {block.label}")
...
def test_manual_integration():
    ...
    try:
        ...
        print("🌐 You can launch this debug app to see if the chatbot UI displays correctly")
...
if __name__ == "__main__":
    ...
    if success1 and success2:
        ...
        print("If you're still not seeing it in the main app, it might be a visual/CSS issue.")
    else:
