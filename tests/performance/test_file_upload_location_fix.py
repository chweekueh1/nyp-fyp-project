...
def test_file_upload_directory_structure():
    ...
    try:
        ...
        expected_test_path = os.path.join(chatbot_dir, 'test_uploads', 'txt_files', 'testuser')
        
        print(f"  🧪 Expected test path: {expected_test_path}")
        
        # Verify the paths are different and correctly structured
        assert expected_prod_path != expected_test_path, "Production and test paths should be different"
        assert 'uploads' in expected_prod_path, "Production path should contain 'uploads'"
        assert 'test_uploads' in expected_test_path, "Test path should contain 'test_uploads'"
        assert expected_prod_path.endswith('testuser'), "Production path should end with username"
        assert expected_test_path.endswith('testuser'), "Test path should end with username"
        
        print(f"  ✅ Directory structure is correct")
        print(f"  ✅ Production and test paths are separate")
        print(f"  ✅ File upload directory structure: PASSED")
...
def test_backend_file_upload_integration():
    ...
    try:
        ...
        assert 'from gradio_modules.file_classification import save_uploaded_file' in backend_content, \
        ...
        assert 'saved_path, original_filename = save_uploaded_file' in backend_content, \
        ...
        print(f"  ✅ Backend imports save_uploaded_file correctly")
        print(f"  ✅ Backend calls save_uploaded_file with correct parameters")
        print(f"  ✅ Backend handles return values correctly")
        print(f"  ✅ Backend has fallback error handling")
        print(f"  ✅ Backend file upload integration: PASSED")
...
def test_file_classification_module_structure():
    ...
    try:
        ...
        assert 'def save_uploaded_file(file_obj, username: str) -> Tuple[str, str]:' in file_class_content, \
            "File classification should have save_uploaded_file function"
        
        assert 'def get_uploaded_files(username: str) -> List[str]:' in file_class_content, \
            "File classification should have get_uploaded_files function"
        
        # Check that it uses the correct directory structure
        assert "os.path.join(get_chatbot_dir(), 'uploads', username)" in file_class_content, \
            "File classification should use correct production directory structure"
        
        assert "os.path.join(get_chatbot_dir(), 'test_uploads', 'txt_files', username)" in file_class_content, \
        ...
        print(f"  ✅ File classification has required functions")
        print(f"  ✅ File classification uses correct directory structure")
        print(f"  ✅ File classification handles test/production environments")
        print(f"  ✅ File classification module structure: PASSED")
...
def test_file_upload_ui_integration():
    ...
    try:
        ...
        assert "'user': user" in file_upload_content or "'username': user" in file_upload_content, \
        ...
        print(f"  ✅ File upload UI calls backend correctly")
        print(f"  ✅ File upload UI passes correct parameters")
        print(f"  ✅ File upload UI integration: PASSED")
