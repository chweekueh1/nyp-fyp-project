...
def test_production_vs_test_paths():
    ...
    try:
        ...
        assert prod_chat != test_chat, "Production and test chat paths should be different"
        assert prod_class != test_class, "Production and test classification paths should be different"
        assert prod_keywords != test_keywords, "Production and test keywords paths should be different"
        assert prod_db != test_db, "Production and test database paths should be different"
        
        # Verify test paths contain 'test'
        assert 'test_uploads' in test_chat, "Test chat path should contain 'test_uploads'"
        assert 'test_uploads' in test_class, "Test classification path should contain 'test_uploads'"
        assert 'test_data' in test_keywords, "Test keywords path should contain 'test_data'"
        assert 'test_data' in test_db, "Test database path should contain 'test_data'"
        
        # Verify production paths don't contain 'test'
        assert 'test' not in prod_chat.lower(), "Production chat path should not contain 'test'"
        assert 'test' not in prod_class.lower(), "Production classification path should not contain 'test'"
        assert 'test' not in prod_keywords.lower(), "Production keywords path should not contain 'test'"
        assert 'test' not in prod_db.lower(), "Production database path should not contain 'test'"
        
        print(f"  🏭 Production chat path: {prod_chat}")
        print(f"  🧪 Test chat path: {test_chat}")
        print(f"  ✅ Path separation: PASSED")
...
def test_test_environment_detection():
    ...
    try:
        ...
        chat_path, _, _, _ = get_data_paths()
        assert 'test_uploads' not in chat_path, "Should not use test paths without TESTING"
        
        print(f"  🔍 Environment detection working correctly")
        print(f"  ✅ Test environment detection: PASSED")
...
def test_directory_creation():
    ...
    try:
        ...
        assert os.path.exists(test_chat), f"Test chat directory should exist: {test_chat}"
        assert os.path.exists(test_class), f"Test classification directory should exist: {test_class}"
        assert os.path.exists(os.path.dirname(test_keywords)), f"Test keywords parent directory should exist"
        assert os.path.exists(test_db), f"Test database directory should exist: {test_db}"
        
        print(f"  📁 Test directories created successfully")
        print(f"  ✅ Directory creation: PASSED")
...
def test_file_processing_isolation():
    ...
    try:
        ...
        from llm.dataProcessing import dataProcessing, get_current_paths
        ...
        assert 'test_uploads' in CHAT_DATA_PATH, f"CHAT_DATA_PATH should be test path: {CHAT_DATA_PATH}"
        assert 'test_uploads' in CLASSIFICATION_DATA_PATH, f"CLASSIFICATION_DATA_PATH should be test path: {CLASSIFICATION_DATA_PATH}"
        
        print(f"  📂 Chat data path: {CHAT_DATA_PATH}")
        print(f"  📂 Classification data path: {CLASSIFICATION_DATA_PATH}")
        print(f"  ✅ File processing isolation: PASSED")
...
def test_database_initialization_isolation():
    ...
    try:
        ...
        from llm.dataProcessing import initialiseDatabase, get_current_paths
        ...
        if chat_files:
            for file_path in chat_files:
                assert 'test_uploads' in file_path, f"Chat file should be in test directory: {file_path}"
        
        if classification_files:
            for file_path in classification_files:
                assert 'test_uploads' in file_path, f"Classification file should be in test directory: {file_path}"
        
        print(f"  📊 Found {len(chat_files)} chat files in test directory")
        print(f"  📊 Found {len(classification_files)} classification files in test directory")
        print(f"  ✅ Database initialization isolation: PASSED")
...
def test_cleanup_and_reset():
    ...
    try:
        ...
        print(f"  🔄 Reset to production mode successfully")
        print(f"  ✅ Cleanup and reset: PASSED")
