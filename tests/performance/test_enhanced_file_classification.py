...
def test_enhanced_content_extraction():
    ...
    try:
        from gradio_modules.enhanced_content_extraction import (
            check_dependencies, enhanced_extract_file_content
        )
        
        # Check dependencies
        deps = check_dependencies()
        print(f"  📦 Dependencies: pandoc={deps['pandoc']}, tesseract={deps['tesseract']}")
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False) as tmp_file:
            ...
            try:
                ...
                assert result['content'].strip() == test_content.strip(), "Content should match"
                ...
                print(f"  ✅ Enhanced content extraction: PASSED")
...
def test_classification_formatter():
    ...
    try:
        ...
        assert '🟢' in formatted['classification'], "Classification should have green emoji for OFFICIAL(OPEN)"
        assert '**OFFICIAL (OPEN)**' in formatted['classification'], "Classification should be bold"
        assert '🟢' in formatted['sensitivity'], "Sensitivity should have green emoji for LOW"
        ...
        assert '## 📋 Classification Summary' in formatted['summary'], "Summary should have header"
        
        print(f"  ✅ Classification formatted with emojis and styling")
        print(f"  ✅ All required sections present")
        print(f"  ✅ Classification formatter: PASSED")
...
def test_file_classification_integration():
    ...
    try:
        ...
        from gradio_modules.file_classification import extract_file_content, classify_file_content
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False) as tmp_file:
            ...
            try:
                ...
                assert isinstance(extraction_result, dict), "Extraction should return dictionary"
                assert extraction_result['content'].strip() == test_content.strip(), "Content should match"
                
                # Test classification (mock the LLM response)
                classification_result = classify_file_content(extraction_result['content'])
                
                # Verify classification result
                assert isinstance(classification_result, dict), "Classification should return dictionary"
                assert 'classification' in classification_result, "Should contain classification"
                assert 'sensitivity' in classification_result, "Should contain sensitivity"
                assert 'reasoning' in classification_result, "Should contain reasoning"
                assert 'confidence' in classification_result, "Should contain confidence"
                
                print(f"  ✅ Content extracted: {len(extraction_result['content'])} characters")
                print(f"  ✅ Classification: {classification_result['classification']}")
                print(f"  ✅ Sensitivity: {classification_result['sensitivity']}")
                print(f"  ✅ File classification integration: PASSED")
...
def test_dependency_availability():
    ...
    try:
        ...
        print(f"  ✅ Dependency availability check: PASSED")
...
def test_file_classification_interface():
    ...
    try:
        ...
        classification_result, sensitivity_result, file_info, reasoning_result = components[4:8]
        ...
        assert isinstance(results_section, gr.Column), "results_section should be Column"
        assert isinstance(classification_result, gr.Textbox), "classification_result should be Textbox"
        assert isinstance(sensitivity_result, gr.Textbox), "sensitivity_result should be Textbox"
        assert isinstance(file_info, gr.Textbox), "file_info should be Textbox"
        assert isinstance(reasoning_result, gr.Textbox), "reasoning_result should be Textbox"
        assert isinstance(summary_result, gr.Markdown), "summary_result should be Markdown"
        
        print(f"  ✅ Interface has 15 components including enhanced summary")
        print(f"  ✅ All component types are correct")
        print(f"  ✅ File classification interface: PASSED")
