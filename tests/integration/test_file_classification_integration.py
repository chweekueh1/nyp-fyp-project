...
def test_file_upload_and_storage():
    ...
    try:
        ...
        assert original_name == "test_upload.txt", "Original filename should be preserved"
    ...
    try:
        for filename, filepath in test_files.items():
            ...
            if "confidential" in filename:
                assert "CONFIDENTIAL" in content.upper(), "Should contain confidential marker"
            elif "public" in filename:
                assert "PUBLIC" in content.upper(), "Should contain public marker"
            elif "personal" in filename:
                assert "PERSONAL" in content.upper() or "PII" in content.upper(), "Should contain personal data marker"
...
def test_classification_functionality():
    ...
    test_cases = [
        ("Confidential business data", "This document contains confidential financial information and trade secrets."),
        ("Public information", "This is a public announcement available to everyone."),
        ("Personal data", "This document contains personal information including SSN and home address."),
        ("Empty content", ""),
    ]
    ...
    for case_name, content in test_cases:
        ...
        try:
            ...
            print(f"    ⚠️ Classification failed (may be expected if LLM not available): {e}")
