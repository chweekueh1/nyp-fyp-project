    ...
    test_chats = [
        {
            ...
            "messages": [
                ...
                {"role": "assistant", "content": "To create a Python function, use the 'def' keyword followed by the function name and parameters."},
        ...
        },
        {
            ...
            "messages": [
                ...
                {"role": "assistant", "content": "The main differences are scope and hoisting. 'let' has block scope while 'var' has function scope."},
                {"role": "user", "content": "Which one should I use?"},
                {"role": "assistant", "content": "Generally, use 'let' for modern JavaScript as it provides better scoping and prevents common errors."}
            ]
        },
        {
            ...
            "messages": [
                ...
                {"role": "assistant", "content": "Start by identifying entities, their attributes, and relationships. Then normalize the schema."},
                {"role": "user", "content": "What is normalization?"},
                {"role": "assistant", "content": "Normalization is the process of organizing data to reduce redundancy and improve data integrity."}
            ]
        }
    ]
    ...
    try:
        ...
        for message, expected_pattern in test_cases:
            ...
            if message and len(message.strip()) > 3:
                ...
                assert not smart_name.startswith("Chat 20"), f"Should generate meaningful name for '{message}'"
...
def test_smart_chat_creation():
    ...
    try:
        ...
        assert "Binary Search Algorithm" in chat_id or "Implement Binary Search" in chat_id, f"Chat name should be meaningful: {chat_id}"
...
def test_chat_history_search():
    ...
    try:
        ...
        # Create test chats
        created_chats = create_test_chats_with_content()
        username = "test_user"
        ...
        for query, expected_count in search_tests:
            ...
            print(f"  🔎 Query: '{query}' → Found {actual_count} chat(s) (expected {expected_count})")
            
            if expected_count > 0:
                assert actual_count >= expected_count, f"Expected at least {expected_count} results for '{query}', got {actual_count}"
                
                # Verify result structure
                for result in results:
                    assert 'chat_id' in result, "Result should have chat_id"
                    assert 'chat_name' in result, "Result should have chat_name"
                    assert 'matching_messages' in result, "Result should have matching_messages"
                    assert 'match_count' in result, "Result should have match_count"
                    assert result['match_count'] > 0, "Should have at least one match"
            else:
                assert actual_count == 0, f"Expected no results for '{query}', got {actual_count}"
