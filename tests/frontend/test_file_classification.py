...
def test_allowed_extensions():
    ...
    allowed_files = [
        "document.txt", "report.pdf", "data.xlsx", 
        "presentation.pptx", "notes.md", "info.docx"
    ]
    ...
    disallowed_files = [
        "script.py", "executable.exe", "image.jpg", 
        "video.mp4", "archive.zip", "unknown"
    ]
...
def test_file_upload_simulation():
    ...
    with tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False) as tmp_file:
        tmp_file.write("This is a test document for classification.\nIt contains sample text content.")
    ...
    try:
        ...
        assert original_name == "test_document.txt", "Original filename should be preserved"
...
def test_classification_interface():
    ...
    try:
        ...
        expected_count = 10
        assert len(components) == expected_count, f"Expected {expected_count} components, got {len(components)}"
